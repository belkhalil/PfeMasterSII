App.controller('SemestreController', [
		...
		'$http',
		'$q',
		function($scope, SemestreService, NiveauService, $http, $q) {
			var self = this;
			self.semestre = {
				idSemestre : null,
				dateDebut : '',
			...
			self.createSemestre = function() {
				SemestreService.createSemestre(self.semestre).then(
						function(d) {
							self.semestre = d;
							console.log(' id de semestre a ajouter:'
									+ self.semestre.idSemestre + ' au niveau '
									+ self.idSelectedNiveau);
							self.addSemestreToNiveau(self.semestre.idSemestre,
									self.idSelectedNiveau);
			...
			self.addSemestreToNiveau = function(idS, idN) {
				SemestreService.addSemestreToNiveau(idS, idN).then(function(d) {
					self.niveaux = d;
					self.fetchAllSemestre();
					self.reset();
				}, function(errResponse) {
			...
			self.update = function() {
				console.log('updating d\'un Semestre'
						+ self.semestre.idSemestre);
